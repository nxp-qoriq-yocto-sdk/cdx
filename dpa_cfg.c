...
extern int cdx_create_port_fqs(void);
extern int cdx_add_eth_onif(char *name);
extern void add_oh_port_tbl_info(uint32_t fm_index, uint32_t port_index, int td,
                                uint32_t type);
extern int cdx_add_oh_iface(char *name);
extern struct net_device *find_osdev_by_fman_params(uint32_t fm_idx, uint32_t port_idx,
                                uint32_t speed);
extern int cdxdrv_init_stats(void *muram_handle);
...
static int cdxdrv_set_miss_action(uint32_t fm_index)
{
	...
	for (ii = 0; ii < finfo->num_tables; ii++) {
		...
		if((tbl_info->type != ETHERNET_TABLE) && 
		   ...
		   (tbl_info->type != IPV4_REASSM_TABLE) && 
			...
			if (FM_PCD_HashTableModifyMissNextEngine(tbl_info->id,	
				...
				DPA_INFO("%s::changed miss action table %s scheme %p\n",
					__FUNCTION__, tbl_info->name, 
					miss_engine_params.params.kgParams.h_DirectScheme);
			}
#endif
		...
		DPA_INFO("%s(%d) distribution scheme %p, table type %d\n",
			__FUNCTION__,__LINE__, miss_engine_params.params.kgParams.h_DirectScheme, tbl_info->type);
		//adding miss action 
		//get ethernet distribution scheme handle
