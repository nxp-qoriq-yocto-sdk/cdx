...
extern void cdx_rtp_set_hwinfo_fields(PRTPflow pFlow, PSockEntry pFromSocket);
...
int SOCKET4_HandleIP_Socket_Open (U16 *p, U16 Length)
{
	...
	DPA_INFO("dscp: %d, expt_flag %d, family %d, fwmark %d\n",
		SocketCmd.dscp, SocketCmd.expt_flag, 4, SocketCmd.rsvd1);
	...
	DPA_INFO("%s(%d) \n",__FUNCTION__,__LINE__);
	pEntry->hw_stats = FM_MURAM_AllocMem(h_FmMuram, 128, 16);
	if (!pEntry->hw_stats)
	{
...
int SOCKET6_HandleIP_Socket_Open(U16 *p, U16 Length)
{
	...
	pEntry->hw_stats = FM_MURAM_AllocMem(h_FmMuram, 128, 32);
	if (!pEntry->hw_stats)
	{
		DPA_ERROR("%s(%d) FM_MURAM_AllocMem failed\n", __FUNCTION__,__LINE__);
		socket6_free(pEntry);
