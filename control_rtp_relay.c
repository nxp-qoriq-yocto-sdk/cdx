...
extern void *dpa_get_fm_MURAM_handle(uint32_t fm_idx, uint64_t *phyBaseAddr,
					uint32_t *MuramSize); // defined in dpa_cfg.c
...
extern void cdx_ehash_set_rtp_info_params(uint8_t *rtp_relay_param, PRTPflow pFlow, PSockEntry pSocket);
extern void cdx_ehash_update_dtmf_rtp_info_params(uint8_t *rtp_relay_param, uint8_t *DTMF_PT);
...
extern int cdx_create_rtp_conn_in_classif_table (PRTPflow pFlow, PSockEntry pFromSocket, PSockEntry pToSocket);
extern void cdx_ehash_update_rtp_info_params(uint8_t *rtp_relay_param, uint32_t *rtpinfo_ptr);
extern int ExternalHashTableDeleteKey(void *h_HashTbl, uint16_t index, void *tbl_entry);
...
void cdx_rtp_set_hwinfo_fields(PRTPflow pFlow, PSockEntry pFromSocket)
{
	...
	if (pFlow->takeover_flags & SSRC_TAKEOVER)
	{
		...
		hw_flow->rtp_info->SSRC = cpu_to_be32(pFlow->SSRC);
	}
	...
	hw_flow->rtp_info->TimestampBase =	cpu_to_be32(pFlow->TimestampBase);
	hw_flow->rtp_info->flags = cpu_to_be16(flags);
	hw_flow->rtp_info->Seq = cpu_to_be16(pFlow->Seq);
...
static int rtp_flow_add(PRTPflow pFlow, U32 hash, PSockEntry pFromSocket, PSockEntry pToSocket)
{
	...
	cdx_rtp_set_hwinfo_fields(pFlow, pFromSocket);
					...
					PSockEntry pingress_socket, PSockEntry pegress_socket)
{
	...
	cdx_rtp_set_hwinfo_fields(pFlow, pingress_socket);
...
static U16 RTP_Call_Control (U16 *p, U16 Length)
{
	...
	{
		flags = be16_to_cpu(pCall->AtoB_flow->hw_flow->rtp_info->flags);
		flags |= RTP_OFFLOAD_PROCESS_PKT;
		pCall->AtoB_flow->hw_flow->rtp_info->flags = cpu_to_be16(flags);
	}

	if (pCall->BtoA_flow->state)
	{
		flags = be16_to_cpu(pCall->BtoA_flow->hw_flow->rtp_info->flags);
		flags |= RTP_OFFLOAD_PROCESS_PKT;
		pCall->BtoA_flow->hw_flow->rtp_info->flags = cpu_to_be16(flags);
	}
...
#ifdef CDX_DPA_DEBUG
static void display_rtp_info(struct _thw_RTPinfo	*rtp_info)
{
...
//	pflow->SSRC_1_takeover = (((pflow->SSRC_1) == 0) ? 0 : 1);
	...
	if (pflow->takeover_mode & RTP_TAKEOVER_MODE_SSRC_AUTO)
	{
		...
		egress_stats->ssrc_overwrite_value = hw_flow->rtp_info->SSRC;
	}

	cdx_rtp_set_hwinfo_fields(pflow, pSocket);
	cdx_ehash_set_rtp_info_params(hw_flow->ehash_rtp_relay_params, 
								pflow, pSocket);
	...
	flags = be16_to_cpu(old_rtp_info->flags);
...
static PRTCPStats RTCP_get_stats(PRTCPStats sw_stats, PRTCPStats hw_stats, U32 stats_size)
{
	...
	if(hw_stats)
	{
		...
		pStats->state = pStats->state; //be16_to_cpu(pStats->state); state variable modified to 1 byte
	}
...
static U16 RTCP_Query (U16 *p, U16 Length)
{
	...
	if(pRTPCmd->flags)
		RTP_reset_stats((PRTCPStats)pSock->hw_stats, pRTPCmd->flags);
...
static void rtpqos_update_entry(PRTPQOS_ENTRY pEntry)
{
	...
	memcpy(hw_rtpqos->saddr, pEntry->saddr, 4*sizeof(U32)); memcpy(hw_rtpqos->daddr, pEntry->daddr, 4*sizeof(U32));
...
static int RTPQOS_enable_stats(U16 *p, U16 Length)
{
	...
	switch(cmd.stream_type)
	{
		...
		case RLY6:
			...
			#if defined (COMCERTO_2000)
				socket6_update(pSocket6, SOCKET_UPDATE);
			#endif
			}
			break;
