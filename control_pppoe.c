...
extern int insert_pppoe_relay_entry_in_classif_table(struct _tPPPoE_Info *entry);
extern int delete_pppoe_relay_entry_from_classif_table(struct _tPPPoE_Info *entry);
...
static int PPPoE_Handle_Relay_Entry(U16 *p, U16 Length)
{
	...
	case ACTION_REGISTER:
		...
		strcpy(&pEntry->hw_entry.in_ifname[0],&cmd->ipifname[0]);
		DPA_INFO("\r\n incoming interface = %s",cmd->ipifname);
		DPA_INFO("\r\n %s",pEntry->hw_entry.in_ifname);

		strcpy(&pRelayEntry->hw_entry.in_ifname[0],&cmd->opifname[0]);
...
extern int dpa_add_pppoe_if(char *name, struct _itf *itf, 
		struct _itf *phys_itf,
                uint8_t *macaddr, uint16_t session_id);
