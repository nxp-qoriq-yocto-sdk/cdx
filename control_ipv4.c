...
extern int delete_entry_from_classif_table(PCtEntry entry);
...
int ct_add(PCtEntry pEntry_orig, TIMER_HANDLER handler)
{
	...
	if(!(pEntry_rep->status &  CONNTRACK_SEC)||
...
void ct_update_one(PCtEntry pEntry)
{
	...
	//insert entry
	if (!(pEntry->status & CONNTRACK_HWSET) && !IS_NULL_ROUTE(pEntry->pRtEntry))
	{
		int rc;
	...
	if (ct && ct->handle) 
	{
		...
		if (param)
		{
			...
			else if (!IS_NULL_ROUTE(pEntry->tnl_route))
			{
				new_mtu = pEntry->tnl_route->mtu;
				//DPA_INFO("%s(%d) \n",__FUNCTION__,__LINE__);
			}
			//DPA_INFO("%s(%d) orig_mtu %d, new_mtu %d \n",__FUNCTION__,__LINE__, orig_mtu, new_mtu);
			if (orig_mtu != new_mtu)
			{
...
int IPv4_HandleIP_CONNTRACK(U16 *p, U16 Length)
{
	...
	switch(Ctcmd.action)
	{
		...
		case ACTION_REGISTER:
				...
				DPRINT(KERN_ERR "%s:%d entry fwmark : %x cmd fw mark : %x  qno : %d\n", __func__, __LINE__, (unsigned int)pEntry_orig->fwmark, (unsigned int)Ctcmd.fwmark, pEntry_orig->queue);
