...
extern void dpa_release_iflist(void); 
extern int cdx_ioc_set_dpa_params(unsigned long args);
extern int cdx_ioc_dpa_connadd(unsigned long args);
extern int cdx_ioc_dpa_configqos(unsigned long args);
extern int cdx_ioc_create_mc_group(unsigned long args);
extern int cdx_ioc_add_member_to_group(unsigned long args);
extern int cdx_ioc_add_mcast_table_entry(unsigned long args);
	...
	switch (cmd) {
		...
		case CDX_CTRL_DPA_GET_MURAM_DATA:
                ...
                case CDX_CTRL_DPA_QOS_CONFIG_ADD:
                     retval = cdx_ioc_dpa_configqos(args);
                    break;
...
int cdx_driver_init(void)
{
	...
	atomic_set(&cdx_ctrl_open_count, 1);
        cdx_ctrl_cdev_major = register_chrdev(
                                        0,
                                        CDX_CTRL_CDEVNAME,
                                        &cdx_dev_fops);
        if (cdx_ctrl_cdev_major < 0) {
        ...
        cdx_ctrl_dev = device_create( cdx_ctrl_class,
                                        NULL,
                                        MKDEV(cdx_ctrl_cdev_major, 0),
                                        NULL,
					CDX_CTRL_CLS_CDEVNAME);
